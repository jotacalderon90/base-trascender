...
var codemirror_head = CodeMirror(document.getElementById("contenthead"), {mode: "text/html", indentWithTabs: true, tabSize: 4});
var codemirror_body = CodeMirror(document.getElementById("contentbody"), {mode: "text/html", indentWithTabs: true, tabSize: 4});
var codemirror_js = CodeMirror(document.getElementById("contentjs"), {mode: "text/html", indentWithTabs: true, tabSize: 4});
...
const www = function(parent){
	...
	this.directory = new trascender({
		service: {
			total: 		["GET", "/api/filemanager/" + btoa(path) + "/total?host=" + urlParams.get('host')],
			collection: ["GET", "/api/filemanager/" + btoa(path) + "/collection?host=" + urlParams.get('host')],
			read: 		["GET", "/api/filemanager/:id?host=" + urlParams.get('host')],
			update: 	["PUT", "/api/filemanager/:id?host=" + urlParams.get('host')]
		},
		start: function(){
			this.service_upload = this.serviceCreate("POST","/api/filemanager/:id/uploader?host=" + urlParams.get('host'));
			this.getCollection();
			this.parent = parent;
		},
		afterGetCollection: async function(){
			const selected = await this.parent.selector.execute('Seleccione archivo',this.coll);
	...
	/*Editor de contenido*/
	this.contenteditor = new trascender({
		load: function(){
			this.coll = [];
